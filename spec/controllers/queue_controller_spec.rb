  ...
  describe "GET index" do
    ...
    context "One track queued" do
      it "should return the next queued track" do
        $redis.lpush "play_queue", track_one
        get :index
    ...
    context "More than one track queued" do
      it "should return upcoming tracks in FIFO order" do
        $redis.lpush "play_queue", track_one
        $redis.lpush "play_queue", track_two
        $redis.lpush "play_queue", track_three
        get :index
  ...
  describe "POST create" do
    context "item is not already queued" do
      ...
      it "should set a flash message" do
        flash.now[:success].should eq I18n.t('queue.create.success', :position => $redis.lindex("play_queue", track_uri))
      end
    ...
    context "item is already queued" do
      before :each do
        $redis.lindex("play_queue", track_uri)
